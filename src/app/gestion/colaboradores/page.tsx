...
export default function ColaboradoresPage() {
  ...
  async function init() {
    setLoading(true)
    const [pResult, uResult, aResult, oResult] = await Promise.all([
      listProfiles(),
      getUserProfile(),
      getAssets(),
      getMyOrganization()
    ...
    <div className="flex-1 pt-14 pb-20 w-full">
      <div className="p-4 space-y-6">
        <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-zinc-200 shadow-sm">
          <div>
            <h1 className="text-xl font-bold text-zinc-900 leading-tight">Colaboradores</h1>
            <p className="text-xs text-zinc-500 mt-0.5">Gestión de personal de campo y administrativo</p>
          </div>
          ...
          <div className="space-y-2">
            {loading ? (
              <div className="py-12 bg-white rounded-2xl border border-zinc-100 flex flex-col items-center justify-center">
                ...
                <p className="text-zinc-400 text-xs mt-3 font-medium">Sincronizando colaboradores...</p>
              </div>
            ) : filteredProfiles.length === 0 ? (
              <div className="py-12 bg-white rounded-2xl border border-zinc-100 text-center">
                <p className="text-zinc-400 text-sm">No se encontraron colaboradores</p>
              </div>
            ) : (
              filteredProfiles.map((p) => (
                <div key={p.id} className="bg-white p-4 rounded-xl border border-zinc-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 group hover:border-teal-300 transition-colors shadow-sm">
                  ...
                  <div className="flex items-center gap-2 sm:shrink-0 ml-13 sm:ml-0">
...
function UserFormModal({ profile, assets, currentUserRole, onClose, onSuccess }: { 
  ...
  const isEditingOperator = formData.role === 'operador'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (newProfile && isEditingOperator && canManageAssets) {
      const profileId = newProfile.id
      if (selectedAssetId && selectedAssetId !== currentAssignedAsset?.id) {
        await assignOperatorToAsset(selectedAssetId, profileId)
      ...
      <div className="bg-zinc-50 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-2 duration-300">
        <div className="px-6 py-5 border-b border-zinc-200 flex justify-between items-center bg-white">
          <div>
            ...
            <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mt-0.5">{profile ? 'Actualizar Datos' : 'Alta de Personal'}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-4 py-6 space-y-6 w-full">
          ...
          {isEditingOperator && ['superadmin', 'gerente', 'gerencia'].includes(currentUserRole) && (
            <div className="space-y-1.5 border-t border-zinc-100 pt-5">
              <Select
                label="Activo Asignado"
                placeholder="Seleccionar activo..."
                options={[
                  { value: '', label: 'Sin activo asignado' },
                  ...availableAssets.map(asset => ({
                    value: asset.id,
                    label: `${asset.type === 'embarcacion' ? '🚤' : asset.type === 'camion' ? '🚛' : '🛻'} ${asset.name} — ${ASSET_TYPE_LABELS[asset.type] || asset.type}${asset.assigned_operator_id === profile?.id ? ' (actual)' : ''}`
                  }))
                ]}
              ...
              <p className="text-[10px] text-zinc-400 ml-1 mt-1">
                {availableAssets.length === 0 
                  ...
                  : 'El activo asignado quedará vinculado al operador para sus tareas'}
              </p>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-2 pb-2">
            ...
            >
              ...
              {loading ? (profile ? 'GUARDANDO...' : 'REGISTRANDO...') : (profile ? 'GUARDAR CAMBIOS' : 'REGISTRAR COLABORADOR')}
            </button>
          </div>
        </form>
      </div>
